...
var effectiveness: Dictionary = {
	...
	"stellar": {} # I guess I need to specify in the battle script that a Stellar Tera uses regular defensive typing. Or I make it match SV internally and treat the stat itself like Poison.
}

func get_effectiveness(move_type: String, defender_type: String) -> float:
	if effectiveness.has(move_type) and effectiveness[move_type].has(defender_type):
		return effectiveness[move_type][defender_type]
	return 1.0
